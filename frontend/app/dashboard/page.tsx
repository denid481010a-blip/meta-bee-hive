...
function ExportKeyModal({ onClose }: { onClose: () => void }) {
  const { exportWallet } = usePrivyAuth();
  ...
  async function handleExport() {
    ...
    try {
      ...
      setError(e?.message ?? "Не удалось экспортировать кошелёк");
    } finally {
      setLoading(false);
    }
  }
          ...
          <button
            onClick={() => {
              if (address) {
                fundWallet(address, {
                  chain: polygon,
                  asset: { erc20: "0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063" }, // DAI on Polygon mainnet
                  amount: "10",
                });
              }
            }}
