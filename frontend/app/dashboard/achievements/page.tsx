...
export default function AchievementsPage() {
  ...
  const getProgress = (id: string): { unlocked: boolean; progress?: number } => {
    switch (id) {
      ...
      case "earned100":    return { unlocked: false }; // from backend
      case "earned1000":   return { unlocked: false }; // from backend
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3 text-center text-sm">
        <div className="bg-navy rounded-xl p-3 border border-white/10">
          ...
          <p className="text-white/40 text-xs">Активных уровней</p>
        </div>
        <div className="bg-navy rounded-xl p-3 border border-white/10">
          <p className="text-2xl font-bold text-bee-green">{teamSize}</p>
          <p className="text-white/40 text-xs">В команде</p>
        </div>
        <div className="bg-navy rounded-xl p-3 border border-white/10">
          <p className="text-2xl font-bold text-white">{cycles}</p>
          <p className="text-white/40 text-xs">Циклов</p>
        </div>
      </div>
